<template>
    <AdminLayout>
        ...
        >
            ...
            >
                <p class="font-bold text-xl mt-2">Reschedule Appointment</p>
                <p class="text-gray-500 dark:text-gray-400">
                    You can only reschedule appointment if the appointment is
                ...
                <form @submit.prevent="submit" method="POST">
                    ...
                    >
                        ...
                        <option
                            v-for="dentist in props.dentists"
                            :value="dentist.id"
                        >
                            {{ dentist.name }}
                        </option>
...
const calendarOptions = {
    ...
    selectable: true,
    initialDate: props.appointment.schedule,
...
const form = useForm({
    ...
    sex: props.appointment.sex,
    contact: props.appointment.contact,
    service: props.appointment.service,
    dentist: props.appointment.dentist_id,
    // remove the time from props appointment split it to date and time
    date: props.appointment.schedule.split(" ")[0],
    // remove secs in time
    time: props.appointment.schedule
        .split(" ")[1]
        .split(":")
        .slice(0, 2)
        .join(":"),
...
const submit = () => {
    console.log("ok");
    form.post(route("admin.appointment.reschedule", props.appointment.id), {
        onSuccess: () => {
            form.reset();
        },
    });
...
const isInitialTime = (t) => {
    return t === props.appointment.schedule.split(" ")[1];
};
